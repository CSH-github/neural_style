...
def vgg_symbol(data, weight, bias):
    def conv(name, data, num_filter, pad, kernel, stride, no_bias, workspace):
        return mx.sym.Convolution(name=name, data=data, weight=weight[name], bias=bias[name], num_filter=num_filter,
                                  pad=pad, kernel=kernel, stride=stride, no_bias=no_bias, workspace=workspace)

    conv1_1 = conv(name='conv1_1', data=data, num_filter=64, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False,
                   workspace=1024)
    relu1_1 = mx.symbol.Activation(name='relu1_1', data=conv1_1, act_type='relu')
    conv1_2 = conv(name='conv1_2', data=relu1_1, num_filter=64, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False,
                   workspace=1024)
    ...
    conv2_1 = conv(name='conv2_1', data=pool1, num_filter=128, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False,
                   workspace=1024)
    relu2_1 = mx.symbol.Activation(name='relu2_1', data=conv2_1, act_type='relu')
    conv2_2 = conv(name='conv2_2', data=relu2_1, num_filter=128, pad=(1, 1), kernel=(3, 3), stride=(1, 1),
    ...
    conv3_1 = conv(name='conv3_1', data=pool2, num_filter=256, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False,
                   workspace=1024)
