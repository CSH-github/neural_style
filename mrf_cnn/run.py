...
def get_mrf_executor(layer, patch_shape):
    ...
    dist = mx.sym.Convolution(data=data, weight=weight, kernel=(patch_size, patch_size), num_filter=patch_shape[0],
                              no_bias=True)
...
for l in range(args.num_res):
    ...
    nn = mx.nd.zeros([vgg_executor.outputs[l].shape[2] - args.patch_size + 1,
                      vgg_executor.outputs[l].shape[3] - args.patch_size + 1], mx.gpu())
...
for epoch in range(args.epochs):
    ...
    if epoch > args.epochs - 30:
        ...
        vgg_executor.outputs[0][:] *= (args.style_weight[0] + args.content_weight[0]) / np.prod(
            vgg_executor.outputs[0].shape)
    else:
        for l in range(args.num_res):
            vgg_executor.outputs[l][:] -= ass_executors[l].outputs[0]  # grad
            vgg_executor.outputs[l][:] *= (args.style_weight[l] + args.content_weight[l]) / np.prod(
                vgg_executor.outputs[l].shape)
    if epoch % 10 == 0:
        total_loss = 0
        print 'Loss:',
        for l in range(args.num_res):
            tmp = 1. / (args.style_weight[l] + args.content_weight[l]) * mx.nd.sum(
                mx.nd.square(vgg_executor.outputs[l])).asnumpy() * np.prod(vgg_executor.outputs[l].shape)
            print tmp,
            total_loss += tmp
        print total_loss

    vgg_executor.backward(vgg_executor.outputs)
    optimizer.update(0, vgg_executor.arg_dict['data'], vgg_executor.grad_dict['data'] + tv_grad_executor.outputs[0],
                     optim_state)
    if epoch % 10 == 0:
