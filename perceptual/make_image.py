...
def crop_img(im, size):
    im = io.imread(im)
    # if img weight is longer than the required size, we only left the middle part.
...
class Maker():
    def __init__(self, model_prefix, output_shape):
        ...
        args['data'] = mx.nd.zeros([1, 3, s0 + 128, s1 + 128], mx.cpu())
