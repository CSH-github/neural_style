...
def init_executor(batch_size, weights=[1, 1, 1, 1], style_layers=['relu1_1', 'relu2_1', 'relu3_1', 'relu4_1'],
...
def train_style(alpha, model_prefix, img_path, weights=[1, 4, 1, 4], tv_weight=1e-4, max_epoch=1000, lr=1e-2):
    ...
    desc_executor, gram_executors, gene_executor = init_executor(batch_size, weights=weights, style_layers=style_layer,
    ...
    for epoch in range(max_epoch):
        ...
        while True:
            ...
            try:
                for i in range(batch_size):
                    desc_executor.arg_dict['data'][i:i + 1] = preprocess_img(
                        os.path.join(MSCOCOPATH, list_img[selected[i]]), size)
            except:
                continue
            break
        desc_executor.forward()
        ...
        for i in range(batch_size):
            gene_executor.arg_dict['data'][i:i + 1] = preprocess_img(os.path.join(MSCOCOPATH, list_img[selected[i]]),
                                                                     size)
        ...
        loss[len(style_layer)] += alpha * np.sum(np.square(
            (desc_executor.outputs[len(style_layer)] - target_content).asnumpy() / np.sqrt(layer_size))) / batch_size
