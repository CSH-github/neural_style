...
from make_image import preprocess_img, postprocess_img
...
def make_video(video_name, start, end, interval, style, save_name, size=[], batchsize=1):
    ...
    for i in range(6):
        ...
        if 'znoise_%d' % i in args:
            args['znoise_%d' % i] = mx.random.uniform(-250, 250,
                                                      [batchsize, 1, size[0] / 2 ** (5 - i), size[1] / 2 ** (5 - i)],
                                                      mx.gpu())
        ...
        if countdown < 0 and i % interval == 0:
            ...
            if len(buf) == batchsize:
                for j in range(batchsize):
                    for ii in range(6):
                        gene_executor.arg_dict['zim_%d' % ii][j:j + 1] = preprocess_img(buf[j], (
                        size[0] / 2 ** (5 - ii), size[1] / 2 ** (5 - ii)))
